...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.groupingBy(x -> x.getSex().equals(Sex.MALE)));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(x -> x.getEmail().substring(x.getEmail().indexOf('@') + 1)));
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(x -> x.getEmail().substring(x.getEmail().indexOf("@") + 1).equals(emailDomain));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                .flatMap(x -> Stream.of(x.getFirstName(), x.getLastName()))
