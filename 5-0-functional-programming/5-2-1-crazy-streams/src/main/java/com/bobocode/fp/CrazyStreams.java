...
public class CrazyStreams {
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                ...
                .map(s -> s.indexOf('/') >= nameLengthBound ? s : s.substring(s.indexOf('/')))
                .map(s -> s.substring(s.indexOf('/')).length() - 1 >= nameLengthBound ? s : s.substring(0,s.indexOf('/')))
                .flatMapToInt(s-> s.replace("/","").toLowerCase(Locale.ROOT).chars())
                .mapToObj(c -> (char) c)
                .collect(Collectors.groupingBy(c -> c, Collectors.counting()));
    }
