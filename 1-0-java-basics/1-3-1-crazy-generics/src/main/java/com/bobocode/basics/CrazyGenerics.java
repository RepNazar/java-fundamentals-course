    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream()
                    .anyMatch(baseEntity -> baseEntity.getUuid() == null);
        }
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            return entities.stream().filter(e -> e.getUuid().equals(targetEntity.getUuid()))
                    .count() > 1;
        }
        ...
        private static <T> void swapHelper(List<T> list, int i1, int i2) {
            T temp = list.get(i1);
