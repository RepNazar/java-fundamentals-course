...
public class CrazyGenerics {
    ...
    public static class MaxHolder<T extends Comparable<? super T>> { // todo: refactor class to make it generic
        ...
        public void put(T val) {
            max = val.compareTo(this.max) > 0 ? val : max;
        }
    ...
    interface ComparableCollection<E> extends Collection<E>, Comparable<Collection<?>> {
        default int compareTo(Collection<?> collection) {
            return this.size() - collection.size() == 0 ? 0 : this.size() - collection.size() > 0 ? 1 : -1;
        }
    }
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().anyMatch(baseEntity -> Objects.isNull(baseEntity.getUuid()));
        }
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            if (Objects.isNull(targetEntity) || Objects.isNull(entities) || entities.isEmpty()) {
                return false;
            }
            return entities.stream().filter(t -> t.getUuid().equals(targetEntity.getUuid())).count() > 1;
        }
