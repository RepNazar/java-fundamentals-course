...
public class CrazyGenerics {
     ...
     */
    interface StrictProcessor<T extends Comparable<Serializable>> { // todo: make it generic
        void process(T obj);
    }
    ...
    interface ComparableCollection<E> extends Comparable<Collection<?>>, Collection<E> { // todo: refactor it to make generic and provide a default impl of compareTo
        ...
        default int compareTo(Collection<?> collection) {
            return collection.size() - this.size() == 0 ? 0 : collection.size() - this.size() > 0 ? 1 : -1;
        }
    }
    ...
    static class CollectionUtil {
         ...
         */
        public static <T extends BaseEntity> boolean hasDuplicates(T targetEntity, List<T> entities) {
            //Equals Error
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            return entities.stream().max(Comparator.comparing(BaseEntity::getCreatedOn)).orElseThrow(NoSuchElementException::new);
        }
